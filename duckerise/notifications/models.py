...
class FollowUpEvent(models.Model):
    ...
    after_event: NotificationEvent = models.ForeignKey(
        NotificationEvent,
        on_delete=models.CASCADE,
        related_name="followup_events",
    )
    delay: int = models.PositiveIntegerField(
        help_text=_("The number of hours this event fire from related event")
    )
    frequency: int = models.PositiveIntegerField(
        help_text=_("This event will fire in every x hours")
    )
    count: int = models.PositiveIntegerField(
        help_text=_("The number of times this event will fire")
    )
